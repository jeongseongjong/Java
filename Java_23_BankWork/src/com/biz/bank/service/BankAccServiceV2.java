...
import com.biz.bank.exec.domain.BankAccVO;
...
public class BankAccServiceV2 implements BankAccService {
	...
	/*
	 * 멤버변수 영역에 선언된 변수 멤버변수 또는 필드 라고 부르며 여기에 선언된 변수들은 프로젝트가 실행되어서 작동되는 동안 그 값을 계속해서
	 * 유지하고 있다.
	 */
	Map<String, BankAccVO> bankList;

	/*
	 * viewAcc() method에서 bankList를 계좌번호로 조회를 했을때 해당하는 계좌번호가 조회되면 bankList에서 해당하는
	 * 계좌정보를 담고있을 인스턴스 변수이다. 또한 이 인스턴스가 가지고 있는 멤버변수들을 변경(수정)하면 그 정보는 bankList에도 반영이
	 * 된다.
	 * 
	 * bankAccVO의 balance, lastDatem, 등 정보를 업데이트하면 bankList에 다시 담지 않아도 자동으로
	 * bankList에 반영이 된다.
	 */
	BankAccVO bankAccVO;
	Scanner scanner;
	...
	public void input() throws Exception {
		...
		if (bankAccVO != null) {
			...
			/*
			 * 지금 bankAccVO의 balance 필드에 값을 변경하면 bankList에 자동으로 현재값이 반영이 된다.
			 * 
			 * bankAccVO값을 변경한 후 bankList에 저장하거나 하는 코드를 사용할 필요가 없다.
			 */
			...
			// DecimalFormat
			// 숫자값 3000 = 3,000 변경시 사용했던 클래스와 비슷한기능
